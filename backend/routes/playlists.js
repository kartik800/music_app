...
router.put("/edit/:id", [validObjectId, auth], async (req, res) => {
  // changing value of database
  const schema = Joi.object({
    name: Joi.string().required(),
    desc: Joi.string().allow(""),
    img: Joi.string().allow(""),
  });
  // validate the body if error occurs send error, otherwise find playlist by id and if so
  // then find corresponding user and match with playlist if so then edit that value and save it;
...
router.put("/add-song", auth, async (req, res) => {
  ...
  // validate the body if error occurs send error, otherwise find playlist by id and if so
  // then find corresponding user and match with user playlist if so then add that value and save it;
  const { error } = schema.validate(req.body);
  if (error) return res.status(400).send({ message: error.details[0].message });
...
router.put("/remove-song", auth, async (req, res) => {
  ...
  // validate the body if error occurs send error, otherwise find playlist by id and if so
  // then find corresponding user if so then remove that value and save it;
  const { error } = schema.validate(req.body);
  if (error) return res.status(400).send({ message: error.details[0].message });
